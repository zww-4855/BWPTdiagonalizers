...
def defineAllVariables(Amat,matrixDim,blockDim,H0def): #defines single vector p,q,eps,H0,V from a given matrix and its dimension
...
def createT0_projQ(projQ,qDim,H0Dim,eps,H0,order,H0def): #qDim>0
    ...
    if H0def=='PHP':
        ...
        QH0Q=(projQ@H0)@projQ
        testZeros=np.zeros((H0Dim,H0Dim))
        print('Check QH0Q != 0 for this choice of H0==diag(H): ',np.allclose(QH0Q,testZeros))
        #eps[0]=eps[0]+0.1
        epsMat=np.zeros((H0Dim,H0Dim))
        denom=np.zeros((H0Dim,H0Dim))
        np.fill_diagonal(epsMat,eps)
...
def runRedefiningH0_lowOrder(H,Hdim,rootTarget,maxorder,blockDim,maxiter,H0def,TOL):
    # H0def has two defn: 
    ...
    for xx in range(1):    
        ...
        for i in range(maxiter):
            ...
            for order in range(0,maxorder):
                print('Inside inner loop with current order: ',order+1, 'out of a max. order: ',maxorder+1)
                if rootTarget!= -1: ## rootTarget intends to target particular root, ie 0th, 1st, 2nd, etc...
                        _,T0=createT0_projQ(projQ,qDim,Hdim,eps[rootTarget],H0,order,H0def)
                        pertVec=extendPSpace(T0,H,p[:,rootTarget],order) # first order correction
                        #order=order+1
        ...
        #                 print('shape of fullSpace: ',np.shape(fullSpace))
                    ...
                    for aa in range(blockDim):
                        if aa in convergedIndx:# dont expect this vector as its converged already
                            continue
                        else:                            
                            _,T0=createT0_projQ(projQ,qDim,Hdim,eps[blockIndx],H0,order,H0def)
                            pertVec=extendPSpace(T0,H,p[:,blockIndx],order) # first order correction
                            pertVec=np.reshape(pertVec,(Hdim,1))
                            var=blockDim+order*blockVar+blockIndx
                            blockIndx=blockIndx+1
                            print('var storing fS locaiton is: ',var)
                            fullSpace[:,[blockDim+order*blockVar+blockIndx]]=pertVec
...
#            print('ritzVecs:',ritzVecs)
            ...
            for zz in range(blockDim):
                fatPhi[:,zz]=qq[:,:var+1]@ritzVecs[:,zz]  # approx to the lowest energy eigenvector
...
#            print('Redefined H0: \n',H0)
            ...
            if rootTarget!= -1: # we are targeting a specific root
                transformedVec=fatPhi[:,rootTarget]   #fullSpace[:,:order+1]@ritzVecs[:,0]
                ...
                if normResid < TOL:
                    print('\n\n\n Converged in micro-iteration: ',(order+1)*(i+1),' macro-iteration:',i+1)
                    print('Converged root ',rootTarget,' :',np.sort(ritzRoots)[rootTarget])
                ...
                for zzz in range(blockDim):
                    if zzz in convergedIndx: # don't incorporate converged roots
                        continue
                    transformedVec=fatPhi[:,zzz]   #fullSpace[:,:order+1]@ritzVecs[:,0]
                    ...
                    if normResid < TOL:
                        print('\n\n\n Converged on iteration: ',(i+1)*order,rootTarget, zzz)
                        print('Converged root ',rootTarget,' :\n\n\n',np.sort(ritzRoots)[zzz])
