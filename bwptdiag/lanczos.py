...
def Get_LanczosResids(HR, R, roots, vecs, i, vecsLength):
    ...
    for jj in range(vecsLength):
        ...
        residEqns = np.linalg.norm(residEqns) / np.linalg.norm(
            R[:, 1 : i + 1] @ vecs[:, jj]
        )
...
def Run_Lanczos(
    ...
    def printLanczos_Initial():
        ...
        print(
            "Checking matrix & vector arrays are storing information via np.sum() ..... "
        )
    ...
    for i in range(1, maxitr + 1):
        ...
        if i != 1:
            ...
            if PreLanczosBOOL and -1.0 * roots[0] - resid > TOL:
                print(
                    "Success! Standard Lanczos has reached the Preconditioned Lanczos stopping criteria in ",
                    i,
                    " iterations.",
                )
