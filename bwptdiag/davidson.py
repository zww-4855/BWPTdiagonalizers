...
def linearEqSolnPRECOND(invBOOL,H,H0,Phi,HPhi,guessE,Hdim,PspaceDim,b,normResid,maxiter,matrixVecMultiplies):
    ...
    if invBOOL=='QHQ':
        ...
        xx=LGMRES.solve(A,-b,maxiter=3,tol=normResid*1e-2,outer_k=3,inner_m=5,M=M,x0=x0)
        #print('lgmres info: ',info)
        print('Checking (E-QH0Q)*x == b aka (E-QH0Q)|r>=|psi>',np.allclose(A@xx,-b[:,0],rtol=10**-4,atol=10**-4),
              np.linalg.norm(xx),np.linalg.norm(b))
...
def GetPrecon_A(Phi,H_Phi,PspaceDim,H,Hdim,guessE):
    ...
    for i in range(Hdim):
        Precon_A[i,]=Precon_A[i,]+guessE-H[i,i]
        Precon_A[i,]=Precon_A[i,]+np.dot(Phi[i,:],Phi[i,:])*H[i,i]
        Precon_A[i,]=Precon_A[i,]+np.dot(H_Phi[i,:],Phi[i,:])
        Precon_A[i,]=Precon_A[i,]-(np.dot(Phi[i,:],Phi[i,:])**4)*H[i,i]
    finalPrecond_A=np.diag(np.diag(Precon_A)) 
    
    projP=Phi@Phi.T
    testPrecon=-H+guessE*np.eye(Hdim)-(projP@H)@projP +projP@H+H@projP
...
def Get_matmulT0guessPsi(Phi,H_Phi,PspaceDim,H,Hdim,guessE,guessPsi):
    ...
    for j in range(PspaceDim):
        for i in range(PspaceDim):
            PHP_Psi=PHP_Psi+Phi[:,j]*(np.dot(Phi[:,j],H_Phi[:,i]))*(np.dot(Phi[:,i],guessPsi))
...
def addRecursiveCorrections(R,matrixDim,PTorder,V,H0,theta,target=0,H0def='diag'):
    finalR=R
    tmpR=R
    p=R
...
def Get_solnSpace(H,R,HR,rootTotal,rootTarget,target,vecs,i,Hdim,lowestElements,seedUnitVecGuess=False):
    if seedUnitVecGuess==False:
        ...
        tmp1[lowestElements[target+1],]=1.0
        ...
        print(R[:,:target+3])
        print(HR[:,:target+3])
        print('matrix comparing HR and R magnitudes: \n',HR[:,:target+2].T@R[:,:target+2])
...
def Get_residual(H,R,HR,vecs,i,matrixDim,blockDim,p,theta,target=0):
    ...
    if i==0:
        ...
        residual[:,0]=HR[:,:i]@vecs[:,target]-theta*(R[:,:i]@vecs[:,target])
...
def Do_SubSpaceCollapse(H,R,HR,Tvecs,currentDim,numVecs_keep,rootsFound,matrixDim): #rootsFound==target is the current root we are trying to target
    R_queue=deque()
    HR_queue=deque()
    #totalSSdim=currentDim+1+rootsFound
    print('@Do_SubSpaceCollapse: current, total SS dim:',currentDim+1)
    print('BUT keeping latest ',numVecs_keep,' vectors in the S.S and keeping: ',rootsFound, 'eigenvector solutions')
    print('@Do_SubSpaceCollapse Does *NOT* include vectors already found; these are kept by default')
...
def Initialize_LinearEqnSolv(invBOOL,Amat,H0,R,HR,theta_p,matrixDim,PspaceDim,residual,normResid,matrixVecMultiplies):
    print("Inverting the full resolvent operation",invBOOL)
    solnR,matrixVecMultiplies=linearEqSolnPRECOND(invBOOL,Amat,H0,R,HR,\
                                             theta_p,matrixDim,PspaceDim,residual,normResid,\
...
def Get_BWPT_Expansion(residual,p,theta_p,H0,H,V,matrixDim,matmuls,BWPTorder=1,target=0):
    # calculate T0|r> & T0|p>
    #if matmuls==1:
        #print('modifying p')
    ...
    print('shape of T0s: ',np.shape(T0_R),np.shape(T0_p),np.linalg.norm(T0_p),np.linalg.norm(T0_R))
    ...
    if BWPTorder >1:
        ...
        alpha=(p[:,0].T@T0_R + p[:,0].T@T0_VT0_R)/(p[:,0].T@T0_p + p[:,0].T@T0_VT0_p)
        print('alpha: ',alpha)
...
def Run_DavidsonBWPT(Amat, matrixDim, blockDim,outputFileName, p=np.asarray([]),theta=None,TOL=10**-6,\
                     maxItr=20,solnCount=1,H0def='diag',highOresolvent=OrderedDict(),invBOOL="QHQ",\
                    numSSVecs_keep=1,maxSSvecs=5,iterH0def="dynamic_QH0Q",spectrum='lowest'):#'static_QH0Q'):
    ...
    for target in range(solnCount):
        ...
        print()
        RootTarget=target
        residual,normResid=Get_residual(Amat,R,HR,vecs,0,matrixDim,blockDim,p,theta_p,RootTarget)
        ...
        for j in range(maxItr):
            ...
            if Query_SubSpaceCollapse(i,maxSSvecs,target):
                R,HR,i=Do_SubSpaceCollapse(Amat,R,HR,vecs,i,numSSVecs_keep,target,matrixDim)
            ...
            if iterH0def=="dynamic_QH0Q":#and (i>0 and i<10) and normResid>10**-5:
                ...
                sigma=theta_p-gap_estimate # gap_estimate is estimate ratio of evalMAX/evalMIN
                ritzVecs_SetBKUP=ritzVecs_Set.copy()
                R[:,target+i+1],matrixVecMultiplies=Initialize_LinearEqnSolv(invBOOL,Amat,H0,new_p,HR[:,target+i],
                                                                             sigma,matrixDim,PspaceDim,residual,normResid,
                                                                             matrixVecMultiplies)
            elif iterH0def=='static_QH0Q':
                R[:,target+i+1],matrixVecMultiplies=Get_BWPT_Expansion(residual,new_p,theta_p,H0,Amat,V,matrixDim,matrixVecMultiplies,
                                                                       BWPTorder=1,target=target)
            ...
            elif H0def=='TriDiag' or H0def=='dynamicH0Diag' or H0def=='BiDiag' or H0def=='QuadDiag':
                ...
                R=Get_CorrVec(R,residual,theta_p,H0,matrixDim,target+i+1,0,H0def)
            ...
            RootTarget=CheckSubspaceCollapse(convRoots, theta[:target+2],RootTarget)
            ...
            residual,normResid=Get_residual(Amat,R,HR,vecs,target+i+2,matrixDim,blockDim,p,theta_p,RootTarget)
            new_p=R[:,:target+i+2]@vecs[:,target] # this is our current Ritz vector; used in J-D procedure
            ...
            if checkConvergence(normResid,TOL,matrixVecMultiplies,theta_p,RootTarget):
                ...
                print('total E from parts: ',(residual.T@fresolv)@residual+(fevecs.T@Amat)@fevecs)
                ...
                if solnCount!=target+1:
                    R,HR=Get_solnSpace(Amat,R,HR,rootTotal,RootTarget,target,vecs,i+2,matrixDim,lowestElements,seedUnitVecGuess)
                theta_p=theta[target+1]#HR[:,target+1].T@R[:,target+1]#theta[target+1]
    ...
    if solnCount==3:
        sortedInfo=postProcessInfo(outputFileName, overallMatMul,thetaRecord, residRecord)
