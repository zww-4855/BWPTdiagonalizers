...
def Get_p(Amat, matrixDim, blockDim, DEBUG=True, iterH0def=""):
    ...
    for x in range(blockDim):
        guessEps.append(
            diagMat[lowestElements[x]]
        )  # -0.001) # seed starting E's to be smaller than diagonal elements
...
def Get_H0(Amat, p, matrixDim, blockDim, H0def, blockSize=0):
    ...
    if H0def == "PHP":
        ...
        print(
            "Not a viable partitioning choice; please choose from / PHP / or / diag /"
        )
        sys.exit()
...
def Get_InitialSpace(Amat, p, theta, Hdim, blockDim, maxItr):
    HR = np.zeros(
        (Hdim, blockDim * maxItr + 1)
    )  # stores all matrix vector multiplies of H w/ the vectors in R
    R = np.zeros(
        (Hdim, blockDim * maxItr + 1)
    )  # stores {|p>, and all correction vecs |Phi>}
