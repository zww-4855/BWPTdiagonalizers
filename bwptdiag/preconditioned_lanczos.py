...
def Get_Mk(H,H0,E,p,dim,QHQapprox="diag",customMk=np.array([]),i=1,rootTarget=0,convgVecs=np.array([]),shift=0.25):
    if customMk.size!=0:
        if i==1:
            return customMk
        else:
...
def Get_CholeskyFactMk(Mk,dim,QHQapprox="diag",H0def="diag"):
    if QHQapprox=="diag" or QHQapprox=="moment" or QHQapprox=='updateH0' or QHQapprox=="constH0shift":
        if H0def=="diag":
...
def Get_TransMatWk(H,dim,E,Lk,QHQapprox="diag",H0def="diag",rootTarget=0,convgVecs=np.array([]),shift=0):
    if QHQapprox=="moment":
    ...
    if QHQapprox=="diag" or QHQapprox=="moment" or QHQapprox=='updateH0' or QHQapprox=="constH0shift" and H0def=="diag":
...
''' 
def Get_MatrixShifts(convgVecs,convgThetas,Amat,dim,shiftList,nextItrTheta,rootTarget):
...
def Run_PrecondLanczos(Amat,matrixDim,H0def="diag",microItr=3,macroItr=10,TOL=10**-5,p=np.array([]),
                       QHQapprox="diag",PreLanczosBOOL=True,customMk=np.array([]),solnCount=1):
    ...
    for rootTarget in range(solnCount):
        ...
        if rootTarget==0:
            ...
            Amat=Amat+convgTheta[rootTarget-1]*convgVecs[rootTarget-1]@convgVecs[rootTarget-1].T
            rootsu,bsroots=eigh(Amat)
        ...
        for i in range(1,macroItr+1):
            ...
            Mk=Get_Mk(Amat,H0,theta,p,matrixDim,QHQapprox,customMk,i,rootTarget,convgVecs,shift=0.0) # Mk is approx to (H-E)
            print('mk diag',np.diag(Mk))
            Lk=Get_CholeskyFactMk(Mk,matrixDim,QHQapprox,H0def) #Mk=Lk@Lk.T
            print('lk diag: ', np.diag(Lk))
            Wk=Get_TransMatWk(Amat,matrixDim,theta,Lk,QHQapprox,H0def,rootTarget,convgVecs,shift=shiftMAT)#Wk=Lk^-1@(H-E)@Lk^-T
            ...
            #       matrix, Wk
            convgeItr,eVec,ResidList,SSevalList,nextTargeteVec=Run_Lanczos(Wk,matrixDim,microItr,
                                                                           np.reshape(yk,(matrixDim,1)),10**-5,
                                                                           2,PreLanczosBOOL,spectraType='lowest')
            ...
            print('new H0 \n',H0)
            if norm < TOL:
                print('\n ** PRECONDITIONED LANCZOS CONVERGED LOWEST ROOT ON ITERATION: ',i)
